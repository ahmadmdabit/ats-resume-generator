...
/**
 * Cross-platform packaging script for ats-resume-generator using Bun's --compile.
 *
 * Usage:
...
Package ats-resume-generator as standalone executables using Bun build --compile.
...
async function main() {
  ...
  // Entry point: compile from src/index.ts (Bun handles TypeScript natively)
  ...
  for (const key of selected) {
    ...
    const outName = `ats-resume-generator-${key}${suffix}`;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        // Copy WASM file for PDF generation alongside the binary
        const wasmSource = resolve(rootDir, "node_modules", "docx-to-pdf-wasm", "build", "docx-to-pdf.wasm");
        const wasmDest = resolve(outdir, "docx-to-pdf.wasm");
        try {
          await access(wasmSource);
          await copyFile(wasmSource, wasmDest);
          console.log(`✅ ${key}: WASM bundled ${wasmDest}`);
        } catch (wasmErr) {
          console.warn(`⚠️  ${key}: WASM file not found at ${wasmSource}, PDF generation will be unavailable at runtime`);
        }
